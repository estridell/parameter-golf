#!/usr/bin/env python3
"""
C11: GPTQ Self-Generated Calibration
=====================================
Use the model's own training data as calibration for GPTQ.
Layer-by-layer quantization with Hessian-based correction.
No external calibration data needed.
"""
...
def log(msg):
    print(f"[C11-GPTQ] {msg}", flush=True)
...
def collect_hessians_fixed(model, train_loader, h, device, n_calibration_batches=64):
    """Fixed version of collect_hessians that handles DocumentPackingLoader's 4-value return."""
    hessians = {}
    hooks = []
...
def main():
    t0 = time.time()
    log("=" * 60)
    log("C11: GPTQ Self-Generated Calibration")
    log("=" * 60)
    ...
    with open("experiments/c11_results.json", "w") as f:
        json.dump(results, f, indent=2)
    log("Results saved to experiments/c11_results.json")
