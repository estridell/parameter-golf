...
TQ_FALLBACK_CODEBOOK = math.sqrt(2.0) * torch.erfinv(_tq_p * 2.0 - 1.0)
...
def use_tq_matrix(name: str, t: Tensor) -> bool:
    return (
        t.ndim == 2
        and min(t.shape) >= TQ_MATRIX_MIN_SIDE
        and name.startswith("blocks.")
        and any(name.endswith(s) for s in TQ_SAFE_SUFFIXES)
    )
...
def tq_rotate(name: str, t: Tensor) -> tuple[Tensor, Tensor, tuple[int, int]]:
    x = t.float()
    rows, cols = x.shape
    norms = x.norm(dim=1)
    ...
    if n != cols:
        y = F.pad(y, (0, n - cols))
    return (
        fht(y * tq_signs(name, n)),
        norms.to(dtype=INT8_PER_ROW_SCALE_DTYPE).contiguous(),
        (rows, cols),
    )


def fit_tq_codebook(state_dict: dict[str, Tensor], names: list[str]) -> Tensor:
    if not names:
        return TQ_FALLBACK_CODEBOOK
    parts = []
    per = max(TQ_SAMPLE_MAX // len(names), 1)
    for name in names:
        z, _, _ = tq_rotate(name, state_dict[name].detach().to("cpu").contiguous())
        a = z.abs().view(-1)
        if a.numel() > per:
            a = a[:: max((a.numel() + per - 1) // per, 1)][:per]
        parts.append(a)
    s = torch.cat(parts).clamp_min_(1e-8)
    k = TQ_LEVELS // 2
    c = torch.quantile(s, (torch.arange(k, dtype=torch.float32) + 0.5) / k).clamp_min_(
        1e-6
    )
    for _ in range(4):
        idx = torch.bucketize(s, (c[1:] + c[:-1]) * 0.5)
        c = (
            torch.bincount(idx, weights=s, minlength=k)
            / torch.bincount(idx, minlength=k).clamp_min_(1)
        ).clamp_min_(1e-6)
    c = c.sort().values
    return torch.cat((-c.flip(0), c))


def quantize_tq_matrix(
    name: str, t: Tensor, edges: Tensor
) -> tuple[tuple[int, int], Tensor, bytes]:
    z, norms, shape = tq_rotate(name, t)
    return shape, norms, pack6(torch.bucketize(z, edges))


def dequantize_tq_matrix(
    name: str,
    shape: tuple[int, int],
    norms: Tensor,
    blob: bytes,
    ...
    rows, cols = shape
    n = 1 << (cols - 1).bit_length()
    z = codebook[unpack6(blob, (rows, n)).long()]
    x = (fht(z) / n) * tq_signs(name, n)
    return (
        (x[:, :cols] * norms.to(dtype=torch.float32)[:, None])
        .to(dtype=dtype)
        .contiguous()
    )
...
def quantize_state_dict(state_dict: dict[str, Tensor]):
    ...
    tq_names = [
        name for name, tensor in state_dict.items() if use_tq_matrix(name, tensor)
    ]
    codebook = fit_tq_codebook(state_dict, tq_names)
    ...
    for name, tensor in state_dict.items():
        ...
        if use_tq_matrix(name, t):
            shape, norms, blob = quantize_tq_matrix(name, t, edges)
...
def dequantize_state_dict(obj: dict[str, object]) -> dict[str, Tensor]:
    out: dict[str, Tensor] = {}
    codebook = obj.get("c", TQ_FALLBACK_CODEBOOK).to(dtype=torch.float32)
    passthrough_orig_dtypes = obj.get("po", {})
    for name, (shape, norms, blob) in obj["m"].items():
        out[name] = dequantize_tq_matrix(
            name, shape, norms, blob, codebook, getattr(torch, obj["d"][name])
        )
...
def main() -> None:
    ...
    if master_process:
        ...
        ratio = quant_stats["baseline_tensor_bytes"] / max(
            quant_stats["quant_payload_bytes"], 1
        )
        log0(
    ...
    log0(
        f"final_tq6_zlib_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}"
    )
