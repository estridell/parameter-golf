...
FA3_ENABLED = bool(int(os.environ.get('FA3_ENABLED', '1')))
if FA3_ENABLED:
...
TRITON_ENABLED = bool(int(os.environ.get('TRITON_ENABLED', '1')))
if TRITON_ENABLED:
    import triton
...
class Hyperparameters:
    ...
    fused_ce_enabled = bool(int(os.environ.get("FUSED_CE_ENABLED", "1" if TRITON_ENABLED else "0")))
    ...
    if compressor == "pergroup":
        ...
        if not _shutil_check.which("lrzip"):
            print("[train_gpt_sota] lrzip not found, falling back to brotli", flush=True)
            compressor = "brotli"
    ...
    def forward(self, x, q_w, k_w, v_w, out_w, cu_seqlens=None, max_seqlen=0):
        ...
        if cu_seqlens is not None:
            if FA3_ENABLED:
                ...
                # Per-document causal SDPA loop (avoids O(total_len^2) mask)
                y = torch.zeros_like(q)
                q_t = q.transpose(1, 2)  # (B, H_q, T, D)
                k_t = k.transpose(1, 2)
                v_t = v.transpose(1, 2)
                ...
                with sdp_kernel(enable_flash=False, enable_mem_efficient=False, enable_math=True):
                    for di in range(len(cu_seqlens) - 1):
                        s, e = int(cu_seqlens[di]), int(cu_seqlens[di + 1])
                        y[:, s:e] = F.scaled_dot_product_attention(
...
def train_model(h, device, val_data):
    ...
    if FA3_ENABLED:
        ...
        model = base_model
        log('torch.compile disabled (FA3_ENABLED=0, varlen fallback has dynamic shapes)')
    ...
    if h.ttt_enabled:
        ...
        def _fwd_ttt(input_ids, target_ids, lora):
            ...
            if _fwd_ttt_compiled_inner is None:
                _fwd_ttt_compiled_inner = torch.compile(_fwd_ttt_inner, dynamic=True) if FA3_ENABLED else _fwd_ttt_inner
